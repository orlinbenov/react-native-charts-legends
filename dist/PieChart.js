...
var PieChart = /** @class */ (function (_super) {
    ...
    PieChart.prototype.render = function () {
        var _this = this;
        var _a = this.props, _b = _a.style, style = _b === void 0 ? {} : _b, backgroundColor = _a.backgroundColor, _c = _a.absolute, absolute = _c === void 0 ? false : _c, _dd = _a.hasChart, hasChart = _dd === void 0 ? true : _dd, _d = _a.hasLegend, hasLegend = _d === void 0 ? true : _d, _e = _a.avoidFalseZero, avoidFalseZero = _e === void 0 ? false : _e;
        ...
        var slices = chart.curves.map(function (c, i) {
            ...
            if (absolute) {
                ...
                if (total === 0) {
                    ...
                    var percentage = c.item[_this.props.accessor].toFixed(2);
                    value = c.item[_this.props.accessor].toFixed(2) + "%";
            ...
            return (<G key={Math.random()}>
                {hasChart ? (<Path d={c.sector.path.print()} fill={c.item.color}/>) : null}
                {hasLegend && hasChart ? (<Rect width="16px" height="16px" fill={c.item.color} rx={4} ry={4} x={_this.props.width / 2.5 - 29} y={-(_this.props.height / 2.5) +
                    ((_this.props.height * 0.8) / _this.props.data.length) * i +
                    12}/>) : null}
                {hasLegend && hasChart ? (<Text fill={c.item.legendFontColor} fontSize="12" fontFamily={c.item.legendFontFamily} x={_this.props.width / 2.5 - 5} y={-(_this.props.height / 2.5) +
                    ((_this.props.height * 0.8) / _this.props.data.length) * i +
                    12 * 2}>{c.item.name + " " + value}</Text>) : null}

                {hasLegend && !hasChart ? (<Rect width="24px" height="24px" fill={c.item.color} rx={4} ry={4} x={-(_this.props.width / 2 - 46 )} y={-(_this.props.height / 2.5) +
                    ((_this.props.height * 0.8) / _this.props.data.length) * i +
                    12}/>) : null}
                {hasLegend && !hasChart ? (<Text fill={c.item.legendFontColor} fontSize="14" fontFamily={c.item.legendFontFamily} x={-(_this.props.width / 2 - 78 )} y={-(_this.props.height / 2.5 - 6) +
                    ((_this.props.height * 0.8) / _this.props.data.length) * i +
                    12 * 2}>{c.item.name}</Text>) : null}
                {hasLegend && !hasChart ? (<Text fill={c.item.legendFontColor} fontSize="14" fontFamily={c.item.legendFontFamily} x={_this.props.width - _this.props.width / 2 - 56} y={-(_this.props.height / 2.5 - 6) +
                    ((_this.props.height * 0.8) / _this.props.data.length) * i +
                    12 * 2}>{value}</Text>) : null}
            </G>);
        });
